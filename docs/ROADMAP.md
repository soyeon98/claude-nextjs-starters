...
| 파일 | 미구현 내용 | 기능 ID |
|------|-------------|---------|
| `app/api/pdf/[token]/route.ts` | 현재 **501 반환**(TODO 주석에 stale한 `SharedInvoice` 참조 존재). `@react-pdf/renderer` 서버 스트림 PDF 생성 | F005 |
| `components/invoice/invoice-pdf.tsx` (신규) | PDF 문서 컴포넌트 (한글 폰트 등록 포함) | F005 |
...
- [ ] 공개 뷰의 `PDF 다운로드` 버튼이 `/api/pdf/[token]` 호출로 PDF 파일을 다운로드 (501 제거, stale `SharedInvoice` TODO 주석 정리)
- [ ] PDF가 웹 렌더와 동일한 데이터(견적서 번호/클라이언트/항목/총금액)를 포함
- [ ] **한글이 깨지지 않고** 정상 표시 (한글 .ttf 폰트 `Font.register`)
- [ ] 없는/접근 불가/잘못된 페이지 ID 접근 시 안내 페이지 표시 + 홈 링크
- [ ] 위 동작에 대한 Playwright MCP 검증 통과
